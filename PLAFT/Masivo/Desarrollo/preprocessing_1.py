...
if __name__ == "__main__":
    subprocess.check_call([sys.executable, "-m", "pip", "install", "--force-reinstall", "numpy==1.19.5"])
    subprocess.check_call([sys.executable, "-m", "pip", "install", "--force-reinstall", "pandas==0.25.3"])
    subprocess.check_call([sys.executable, "-m", "pip", "install", "--force-reinstall", "pyarrow==0.17.1"])
    subprocess.check_call([sys.executable, "-m", "pip", "install", "--force-reinstall", "dask==2021.7.2"])
    subprocess.check_call([sys.executable, "-m", "pip", "install", "--force-reinstall", "fsspec==2021.7.0"])
...
def LecturaDatos(meses,cols_Exclude=[]):
    df=pd.DataFrame()
    for mes in meses:
        #print("======Lectura Mes: {}".format(mes))
        #start_time = time.time()
        
        df_path = '/opt/ml/processing/input/periodo={}/*'.format(mes)
        df2=dd.read_parquet(df_path)
        
        df2=df2.map_partitions(process_data,'target',False,cols_Exclude).compute()
        df=df.append(df2, ignore_index=True)
        
        #print("--- %s min ---" % ((time.time() - start_time)/60))
        #print("======FinLectura {}".format(mes))
...
#def LecturaDatos(meses, cols_Exclude=[]):
#    df_path = '/opt/ml/processing/input/masivo500k_0s/*.parquet'
...
if __name__ == '__main__':
    ...
    extras = df[['cod_mes', 'key_value' ,'cuc_num']].copy()
