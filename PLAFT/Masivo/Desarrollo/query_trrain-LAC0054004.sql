CREATE TABLE "disc_comercial"."plaft_pn_masivo_train" WITH (
     format = 'parquet',
...
WITH alarmas_positivos AS (
    SELECT
        CAST(a.cod_cli AS VARCHAR)                                          AS cuc_num,
    ...
    WHERE a.p_codmes BETWEEN '202501' AND '202507'
...
clientes_con_actividad AS (
    SELECT
        CAST(cuc_num AS VARCHAR) AS cuc_num,
    ...
    WHERE tipopersona = 'N'
      ...
      AND p_mesinformacion BETWEEN
            DATE_FORMAT(DATE_ADD('month', -1, DATE_PARSE('202501', '%Y%m')), '%Y%m')
        AND DATE_FORMAT(DATE_ADD('month', -1, DATE_PARSE('202507', '%Y%m')), '%Y%m')
      AND DATE_TRUNC('month', fechinievento)
            BETWEEN DATE_ADD('month', -1, DATE_PARSE('202501', '%Y%m'))
                AND DATE_ADD('day', -1, DATE_PARSE('202507', '%Y%m'))
    GROUP BY CAST(cuc_num AS VARCHAR), DATE_FORMAT(DATE_TRUNC('month', fechinievento), '%Y%m')
...
alarmas_nunca_alertado AS (
    ...
    -- El outer SELECT excluye rn para que el schema coincida con alarmas_positivos en el UNION ALL.
    SELECT
    ...
    FROM (
        ...
        FROM d_perm_aws.t_agg_alertas_plaft a
        INNER JOIN clientes_con_actividad ca
            ON  CAST(a.cod_cli AS VARCHAR) = ca.cuc_num
...
base_credito AS (
    ...
    INNER JOIN alarmas_ids bi
        ON CAST(tc.cuc_num AS VARCHAR) = bi.cuc_num
    WHERE tc.trx_limpia_ind = 1
      ...
      AND tc.p_proceso_fc BETWEEN
            DATE_FORMAT(DATE_ADD('month', -1, DATE_PARSE('202501', '%Y%m')), '%Y%m%d')
        AND DATE_FORMAT(DATE_ADD('day', -1, DATE_PARSE('202507', '%Y%m')), '%Y%m%d')
...
base_debito AS (
    ...
    INNER JOIN alarmas_ids bi
        ON CAST(td.cuc_num AS VARCHAR) = bi.cuc_num
...
trx_agregadas AS (
    ...
    FROM (
        ...
        FROM (
            ...
            INNER JOIN alarmas_ids bi
                ON CAST(t.cuc_num AS VARCHAR) = bi.cuc_num
...
v360_reducido AS (
    ...
    INNER JOIN alarmas_ids bi
        ON CAST(v.cuc AS VARCHAR) = bi.cuc_num
...
LEFT JOIN v360_reducido v
    ON  a.cuc_num = CAST(v.cuc AS VARCHAR)
