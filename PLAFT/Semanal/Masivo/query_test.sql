CREATE TABLE "disc_comercial"."plaft_pn_masivo_test_" WITH (
     format = 'parquet',
...
WITH alarmas_positivos AS (
    ...
    WHERE a.p_codmes BETWEEN '202508' AND '202604'
...
clientes_con_actividad AS (
    ...
    WHERE tipopersona = 'N'
      ...
      AND DATE_TRUNC('month', fechinievento)
            BETWEEN DATE_ADD('month', -1, DATE_PARSE('202508', '%Y%m'))
                AND DATE_ADD('day', -1, DATE_PARSE('202604', '%Y%m'))
...
base_credito AS (
    ...
    WHERE tc.trx_limpia_ind = 1
      ...
      AND tc.p_proceso_fc BETWEEN
            DATE_FORMAT(DATE_ADD('month', -1, DATE_PARSE('202508', '%Y%m')), '%Y%m%d')
        AND DATE_FORMAT(DATE_ADD('day', -1, DATE_PARSE('202604', '%Y%m')), '%Y%m%d')
